...
function mostrarError(xhr, statusText, errorThrown, $form) {
    ...
    monoloading('hide');
    if (xhr.status === 401) {
        $form.find(".modal-errores").html("<p>Tu sesión caducó, por favor <a href='/' target='_blank'>inicia sesión</a> e intenta de nuevo</p>");
    } else if (xhr.status === 500) {
        $form.find(".modal-errores").html("<p>Ocurrió un error en el servidor</p>");
    } else {
        $form.find(".modal-errores").html("<p>Ocurrió un error inesperado</p>");
    }
}
...
function monomiddle(icono) {
    $("#alertTop").html("<div class=\"mono-middle scene_element scene_element--fadeindown shadow-z-1\">" + icono + "</div>");
}
...
$(document).on("hidden.coreui.modal", ".modal", function (event) {
    if ($(".modal").hasClass("show")) {
        $("body").addClass("modal-open");
    }
});

$(document).on("hide.coreui.modal", ".modal", function (event) {
